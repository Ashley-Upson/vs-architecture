...
internal static class AdjacentDownwardCommonRailObserver
{
    public static CommonRailParityReport Observe(AdjacentDownwardObservationReport source, int separation, int padding)
    {
        ...
        var through = eligible.Select(item => item.Demands.Single(demand => demand.Role == RailSemanticRole.Through)).ToArray();
        ...
        var regions = through.GroupBy(RegionKey).OrderBy(item => item.Key, StringComparer.Ordinal).Select(group =>
        {
            ...
            return new CommonRailRegionObservation(region,
                DeterministicRailAllocator.Assign(region, group, new RailAssignmentOptions(separation, padding)), null);
        }).ToArray();
        ...
        var byDemand = regions.SelectMany(item => item.Assignment.RailsByDemandId)
            .ToDictionary(item => item.Key, item => item.Value, StringComparer.Ordinal);
        var reconstructionTicks = 0L;
        var parityTicks = 0L;
        var routes = eligible.Select(route =>
        {
            var demand = route.Demands.Single(item => item.Role == RailSemanticRole.Through);
            ...
            var existingDeparture = route.SelectedAssignedRails.Single(item => item.Role == RailSemanticRole.TerminalDeparture);
            var existingArrival = route.SelectedAssignedRails.Single(item => item.Role == RailSemanticRole.TerminalArrival);
            ...
            var transitions = new[]
            {
                new RailTransition($"{route.LogicalRouteId}:common-transition:0", route.LogicalRouteId,
                ...
                new RailTransition($"{route.LogicalRouteId}:common-transition:1", route.LogicalRouteId,
    ...
    private static CommonAssignmentParity Compare(AssignedRail common, AssignedRail existing) =>
        common.LaneIndex == existing.LaneIndex && common.AxisCoordinate == existing.AxisCoordinate
