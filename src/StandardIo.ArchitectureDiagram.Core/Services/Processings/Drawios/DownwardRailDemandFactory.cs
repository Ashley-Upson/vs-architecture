...
internal static class DownwardRailDemandFactory
{
    public static (IReadOnlyList<RailDemand> Demands, IReadOnlyList<int> TransitionXs) Create(
        AdjacentDownwardRouteContext context,
        IReadOnlyList<InterLayerBandId> crossedBands)
    {
        if (crossedBands.Count == 0) return (Array.Empty<RailDemand>(), Array.Empty<int>());
        ...
        var transitionXs = Enumerable.Range(1, crossedBands.Count)
            ...
            .ToArray();
        var demands = new List<RailDemand>
        {
            new($"{context.Route.Link.Id}:rail:departure", context.Route.Link.Id, RailOrientation.Vertical,
                new AxisInterval(context.Route.SourcePoint.Y, context.BandAxisRanges[crossedBands[0]].Maximum),
                new AxisInterval(sourceX, sourceX), sourceX, RailSemanticRole.TerminalDeparture,
        ...
        for (var index = 0; index < crossedBands.Count; index++)
        {
            ...
            demands.Add(new RailDemand(
                crossedBands.Count == 1
                    ? $"{context.Route.Link.Id}:rail:through"
                    : $"{context.Route.Link.Id}:rail:through:{band.UpperLayer}:{band.LowerLayer}",
                context.Route.Link.Id, RailOrientation.Horizontal, new AxisInterval(startX, endX),
                context.BandAxisRanges[band], null, RailSemanticRole.Through, context.Route.Link.Order, index + 1,
                new MovementScopeIdentity(MovementScopeKind.LayerAndLowerSuffix, $"depth:{band.LowerLayer}"),
                context.LayoutRevision, context.RouteRevision));
            startX = endX;
        }
        demands.Add(new RailDemand(
            $"{context.Route.Link.Id}:rail:arrival", context.Route.Link.Id, RailOrientation.Vertical,
            ...
            RailSemanticRole.TerminalArrival, context.Route.Link.Order, crossedBands.Count + 1,
