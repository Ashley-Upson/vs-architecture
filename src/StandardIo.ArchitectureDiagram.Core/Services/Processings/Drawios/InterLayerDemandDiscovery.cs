...
internal static class InterLayerDemandDiscovery
{
        ...
        RouteRevision? expectedRouteRevision = null)
    {
        ...
        var builders = Enumerable.Range(0, maximumLayer)
            ...
            .Select(upper => new InterLayerBuilder(
        ...
        foreach (var link in routes.Links.Values.OrderBy(link => link.Link.Order).ThenBy(link => link.Link.Id, StringComparer.Ordinal))
        {
            ...
            var touched = new List<(InterLayerBuilder InterLayer, int First, int Last)>();
            foreach (var interLayer in builders.Values.OrderBy(item => item.Id.UpperLayer))
            {
                var indices = Enumerable.Range(0, points.Length - 1)
                    .Where(index => IntersectsVerticalRange(points[index], points[index + 1], interLayer.UpperBoundary, interLayer.LowerBoundary))
                    .ToArray();
                if (indices.Length == 0) continue;
                foreach (var group in Contiguous(indices))
                {
                    touched.Add((interLayer, group.First(), group.Last()));
                }
            }

            var orderedTouched = touched.OrderBy(item => item.First).ThenBy(item => item.InterLayer.Id.UpperLayer).ToArray();
            for (var membershipIndex = 0; membershipIndex < orderedTouched.Length; membershipIndex++)
            {
                ...
                var membershipId = $"{link.Link.Id}:interLayer-{item.InterLayer.Id.UpperLayer}:segments-{item.First}-{item.Last}";
                item.InterLayer.Memberships.Add(new InterLayerLinkMembership(
                    membershipId, link.Link.Id, routes.Revision, item.InterLayer.Id, item.First, item.Last, role));
                for (var segmentIndex = item.First; segmentIndex <= item.Last; segmentIndex++)
                {
                    ...
                    {
                        item.InterLayer.UnsupportedShapes.Add($"{link.Link.Id}:segment-{segmentIndex}:non-orthogonal");
                    }
                    if (start.Y != end.Y || start.Y < item.InterLayer.UpperBoundary || start.Y > item.InterLayer.LowerBoundary) continue;
                    item.InterLayer.PendingDemands.Add(new PendingDemand(
                        $"{membershipId}:segment-{segmentIndex}", link.Link.Id, routes.Revision, item.InterLayer.Id,
        ...
        timer.Stop();
        var interLayersByLink = observations.SelectMany(interLayer => interLayer.Memberships)
            .GroupBy(item => item.LogicalEdgeIdentity, StringComparer.Ordinal)
            .Select(group => group.Select(item => item.InterLayerId).Distinct().Count())
        ...
        var telemetry = new InterLayerTelemetry(
            ...
            observations.Sum(interLayer => interLayer.Memberships.Count), interLayersByLink,
            observations.Sum(interLayer => interLayer.Demands.Count), observations.Sum(interLayer => interLayer.OverlapGroupCount),
            observations.Select(interLayer => interLayer.MaximumSimultaneousOverlap).DefaultIfEmpty(0).Max(),
            observations.Sum(interLayer => interLayer.Demands.Count(demand => demand.Role == InterLayerMembershipRole.Return)),
            observations.Sum(interLayer => interLayer.UnsupportedShapes.Count), comparisons,
            (long)(timer.ElapsedTicks * 1_000_000d / Stopwatch.Frequency));
        PerformanceAudit.Increment("inter-layer interLayers observed", telemetry.InterLayerCount);
        PerformanceAudit.Increment("inter-layer interLayer demands", telemetry.HorizontalDemandCount);
        ...
        IReadOnlyList<InterLayerObservation> interLayers)
    {
        ...
        {
            var relevant = interLayers.Where(interLayer => interLayer.Memberships.Any(membership =>
                membership.LogicalEdgeIdentity == finding.EdgeId || membership.LogicalEdgeIdentity == finding.OtherEdgeId)).ToArray();
            var underSized = relevant.Any(interLayer => interLayer.MissingExtent > 0);
            return new InterLayerFindingCorrelation(
                finding.Code, finding.EdgeId, finding.OtherEdgeId, relevant.Select(interLayer => interLayer.Id).ToArray(),
                relevant.SelectMany(interLayer => interLayer.Demands).Where(demand =>
                ...
                underSized ? "At least one associated interLayer has missing required extent." : "Associated interLayers have no missing extent; interLayer expansion alone is unlikely to resolve it.");
        }).ToArray();
    }

    private sealed class InterLayerBuilder
    {
        public InterLayerBuilder(InterLayerId id, int upperBoundary, int lowerBoundary)
        ...
        public InterLayerObservation Build(int clearance, int padding, ref long comparisons)
        {
            ...
            foreach (var roleGroup in PendingDemands.GroupBy(item => item.Role).OrderBy(group => group.Key))
            {
                ...
                slotCount = Math.Max(slotCount, createdSlots);
                if (roleGroup.Key == InterLayerMembershipRole.Return) returnSlotCount = createdSlots;
                else ordinarySlotCount = Math.Max(ordinarySlotCount, createdSlots);
            }
            ...
            var effectiveSlots = returnRegions.Any(item => item.ConflictsWithDownwardTraffic)
                ? ordinarySlotCount + returnSlotCount
                : Math.Max(ordinarySlotCount, returnSlotCount);
            var required = PendingDemands.Count == 0 ? current : padding * 2 + Math.Max(1, effectiveSlots) * clearance;
            return new InterLayerObservation(Id, UpperBoundary, LowerBoundary, current, required,
                ...
                maximumOverlap, Math.Max(slotCount, effectiveSlots), returnSlotCount, returnRegions,
                UnsupportedShapes.OrderBy(item => item, StringComparer.Ordinal).ToArray());
        }
    }

    private sealed record PendingDemand(string Id, string EdgeId, RouteRevision Revision, InterLayerId InterLayerId,
        int SegmentIndex, InterLayerMembershipRole Role, int XStart, int XEnd, int ConnectionOrder, InterLayerLinkDirection Direction)
    {
        public InterLayerLinkDemand ToDemand(int slot) => new(Id, EdgeId, Revision, InterLayerId, SegmentIndex, Role,
            XStart, XEnd, ConnectionOrder, Direction, slot);
        ...
        public LinkSegmentDemand ToLinkSegmentDemand(int upperBoundary, int lowerBoundary) => new(
            ...
            new MovementScopeIdentity(MovementScopeKind.LayerAndLowerSuffix, $"depth:{InterLayerId.LowerLayer}"),
            InterLayerId.LayoutRevision, Revision);
    }
}
