...
public sealed class ArchitectureV7PreRoutingNodeSpanSizer
{
        ...
        ArchitectureV7PrePlacementConfiguration configuration)
    {
        ...
        var requirements = ownership.Projection.PhysicalNodes.Select(node =>
        {
            ...
            var incomingRequirement = TerminalWidth(incoming.TryGetValue(node.PhysicalNodeId, out var topCount) ? topCount : 0, configuration);
            var outgoingRequirement = TerminalWidth(outgoing.TryGetValue(node.PhysicalNodeId, out var bottomCount) ? bottomCount : 0, configuration);
            ...
            var span = Math.Max(3, (int)Math.Ceiling(requiredWidth / (double)configuration.ConfiguredBaseCellWidth));
            if (span % 2 == 0) span++;
            return new ArchitectureV7NodeSpanRequirement(node.PhysicalNodeId, span, labelRequirement,
                ...
                $"v7-pre-routing;label={labelRequirement};incoming={incomingRequirement};outgoing={outgoingRequirement};minimum={configuration.ConfiguredMinimumWidth};base-cell={configuration.ConfiguredBaseCellWidth}");
