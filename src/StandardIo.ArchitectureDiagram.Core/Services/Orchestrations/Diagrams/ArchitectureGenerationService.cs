...
public sealed class ArchitectureGenerationService : IArchitectureGenerationService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult(new TypedArchitectureGenerationResult(
            ...
            new ArchitectureEligibilityResult(false, new[] { "V6 projection, logical placement and abstract routing are available; lane allocation, sizing, geometry and Architecture node/link emission are deferred." }),
            () => new DrawioDiagnosticExportResult(page.GraphModel.ToString(),
                "{\"projectionCompleted\":true,\"logicalPlacementCompleted\":true,\"abstractRoutingCompleted\":true,\"laneAllocationDeferred\":true,\"sizingCompleted\":false,\"absoluteGeometryCompleted\":false,\"sizingDeferred\":true,\"absoluteGeometryDeferred\":true}",
