...
`RenderModel.Rows` records inferred group keys, starting logical rows, and node IDs. Dependencies within a group create additional subrows. Dependencies between groups establish their ordering, including forward cross-project references. Category membership is retained when group ordering conflicts: dependencies can require a later band of the same category, but different categories never share a row. Real cycle handling stays with the existing depth rule.
...
### Exclusive category bands

Rows are now allocated exclusively to one inferred category before horizontal placement. Ready nodes of a category occupy its row together, with extra rows for calls within that category. The scheduler prefers categories whose outside predecessors have all been placed. Where category ordering conflicts, it retains category membership on a later exclusive row rather than falling back into mixed dependency rows. Unmatched types have their own category. `RenderModel.Rows` includes all occupied logical rows, including unmatched rows; keys identify the category and row number. Project tiers omit unused bands consistently across their side-by-side containers.
