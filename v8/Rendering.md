...
### Shared colour categories and repeated bands

`RenderNode.Category` is assigned by the same classifier used for its fill colour. `CategoryRowLayoutRuleProcessingService` consumes that category after dependency depth and before horizontal placement. The former independent prefix/suffix row classifier has been removed, so differently named exposures no longer acquire different bands merely because their names differ.

The existing categories are exposures, orchestrations, processings, foundations, brokers, and other types. External nodes are exposures. Hub, Context, and Client suffixes identify known exposure types as well. Exposure nodes use the exposure palette colour, including external exposures; the seventh legacy palette entry is retained for configuration compatibility.

The scheduler places dependency-ready peers of one category on an exclusive row. When several categories are ready, it prefers the exposure-to-broker order. Dependencies always control readiness: broker-to-exposure calls therefore create a subsequent exposure band. Calls within a category can require additional rows. No empty category rows are allocated. `RenderModel.Rows` records occupied category rows and node IDs.

Projects at the same dependency tier align occupied category rows. Downstream project tiers start fresh and omit bands absent from that tier. Horizontal spacing, centring, compaction, and routing preserve the resulting vertical assignment. Existing cycle handling remains with the dependency-depth rule. This visual classification does not assert architectural compliance.
