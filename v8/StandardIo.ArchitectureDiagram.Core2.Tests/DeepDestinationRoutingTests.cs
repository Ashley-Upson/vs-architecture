...
public sealed class DeepDestinationRoutingTests
{
    ...
    [Fact]
    public void ShouldKeepSharedRootAnchoringFromClosingReservedPassages()
    {
    ...
    [Fact]
    public void ShouldUseDepthPriorityAcrossProjectBoundary()
    {
        RenderNode Node(string id,double x,double y)=>new(id,id,id,"blue",x,y,180,60,[]);
    ...
    [Fact]
    public void ShouldMoveBlockingChildBranchToClearDeepConnection()
    {
    ...
    [Fact]
    public void ShouldNotSendNearCentreChildBackAcrossDeeperExit()
    {
        var drawing=Drawing();
        drawing=drawing with {Nodes=drawing.Nodes.Select(n=>n.Type.Name=="Near"?n with {X=505}:n).ToArray()};
        var routes=DiagramRouting.CreateRoutes(drawing,new RenderConfiguration {NoDuplicates=true});
    ...
    [Fact]
    public void ShouldAllocateDeepDestinationExitBeforeNearLayerExits()
    {
        var routes=DiagramRouting.CreateRoutes(Drawing(),new RenderConfiguration {NoDuplicates=true});
        var near=routes.Single(r=>r.Relationship.ToType=="Near");
        var deep=routes.Single(r=>r.Relationship.ToType=="Deep");
    ...
    [Fact]
    public void ShouldDescendThroughClearLayersBeforeBendingToDeepDestination()
    {
        var route=DiagramRouting.CreateRoutes(Drawing(),new RenderConfiguration {NoDuplicates=true})
