...
internal static class ProjectModelTestFactory
{
    public static ProjectModelOrchestrationService Create(IRoslynBroker? broker = null, IProjectService? projectService = null)
    {
        ...
        return new ProjectModelOrchestrationService(
            ...
            projectTypesProcessingService: new ProjectTypesProcessingService(new ProjectTypesService(broker)),
            projectDependenciesProcessingService: new ProjectDependenciesProcessingService(new ProjectDependenciesService(broker)));
    }

    public static Task<ProjectModel> ExtractAsync(Project project) =>
        Create(broker: new CompilationBroker(project), projectService: new FixtureProjectService()).GenerateProjectModelAsync(projectFilePath: project.FilePath!);
    ...
    private sealed class FixtureProjectService : IProjectService
    {
        ...
        public string[] GetProjectFiles(string directory) => throw new NotSupportedException();
    }
