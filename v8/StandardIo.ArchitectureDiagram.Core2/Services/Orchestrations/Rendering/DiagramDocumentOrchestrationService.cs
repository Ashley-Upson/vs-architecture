...
{
    public byte[] Render(RenderModel model, DiagramFormats format) => compiler.Compile(new[] { DiagramTypes.Architecture, DiagramTypes.Composition, DiagramTypes.DataModel }.Select(type => tabs.Render(model, type, format)).ToArray(), format);
}
