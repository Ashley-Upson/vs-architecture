    ...
    public byte[] Render(RenderModel renderModel)
    {
        ...
        foreach (RenderConnection route in renderModel.CrossProjectConnections)
        {
            ...
            root.Add(new XElement("mxCell", new XAttribute("id", route.Id), new XAttribute("parent", "1"),
                ...
                new XAttribute("style", "edgeStyle=none;noEdgeStyle=1;rounded=0;strokeColor=" + route.Stroke + ";exitX=" + exitX + ";exitY=1;entryX=0.5;entryY=0;exitPerimeter=0;entryPerimeter=0;" + (route.Inheritance ? "endArrow=block;endFill=0;dashed=1;" : route.IsDeferred ? "endArrow=classic;dashed=1;" : "endArrow=classic;")),
        ...
        var file = new XDocument(new XElement("mxfile", new XAttribute("host", "app.diagrams.net"), new XElement("diagram", new XAttribute("id", "architecture"), new XAttribute("name", "Architecture"), new XElement("mxGraphModel", new XAttribute("grid", "1"), new XAttribute("gridSize", "10"), root))));
