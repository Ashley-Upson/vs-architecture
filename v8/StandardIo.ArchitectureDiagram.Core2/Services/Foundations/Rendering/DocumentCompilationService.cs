...
internal sealed class DocumentCompilationService : IDocumentCompilationService
{
    ...
    public byte[] Compile(RenderedDiagramTab[] tabs, DiagramFormats format)
    {
        ...
        for (int i = 0; i < tabs.Length; i++)
        {
            ...
            nav.Add(button);
            var frame = new XElement("iframe", new XAttribute("id", "tab-" + i), new XAttribute("role", "tabpanel"), new XAttribute("title", Name(tabs[i].DiagramType)), new XAttribute("srcdoc", Encoding.UTF8.GetString(tabs[i].Content)));
            if (i != 0) frame.SetAttributeValue("hidden", "hidden");
            body.Add(frame);
        }
        body.Add(new XElement("script", "document.querySelectorAll('[data-tab]').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('[data-tab]').forEach(x=>x.setAttribute('aria-selected',String(x===b)));document.querySelectorAll('iframe').forEach(f=>f.hidden=f.id!=='tab-'+b.dataset.tab);}));"));
