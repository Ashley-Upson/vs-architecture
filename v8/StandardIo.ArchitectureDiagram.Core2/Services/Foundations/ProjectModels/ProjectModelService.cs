...
internal sealed class ProjectModelService : IProjectModelService
{
        ...
        new ProjectModel
    {
        Name = source.Name,
        Path = source.Path,
        Types = types.Select(selector: type => new DefinedType { Name = type.Name, AssemblyName = type.AssemblyName, FrameworkType = type.FrameworkType, IsInternal = type.IsInternal, Fields = (type.Fields ?? Array.Empty<Field>()).Select(selector: field => new Field { Name = field.Name, Type = field.Type })
