...
internal sealed class FileBroker : IFileBroker
{
    ...
    public string GetDirectoryPath(string path) => Path.GetDirectoryName(path: path)!;
    ...
    public string[] GetProjectFiles(string directory) => Directory.GetFiles(path: directory, searchPattern: "*.csproj", searchOption: SearchOption.TopDirectoryOnly);
}
